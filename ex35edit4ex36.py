# Modified my version of the game for Exercise 36, page 95 admonition:
#	"Review the last exercise to see if the If-Statement rules are followed. If not, fix."
	...
	if ate_blueberries:
		...
		while True:
			...
			if 'honey' in next:
				...
				# I'm breaking a rule here (not nesting more than two deep), but the logic is fairly straightforward.
				if has_gun:
...
def cthulhu_room():
	...
	if ate_blueberries:
		...
		if 'flee' in next:
			...
			# Breaking the nesting rule again, but you can see what I'm thinking, rather than calling a function that you have to search for/through.
			if has_gun:
